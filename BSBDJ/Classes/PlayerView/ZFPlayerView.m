...
-(void)setVideoURL:(NSURL *)videoURL
{
    ...
    [self.playerItem addObserver:self forKeyPath:@"loadedTimeRanges" options:NSKeyValueObservingOptionNew context:nil];// 监听loadedTimeRanges属性（视频缓冲进度）
    // 添加手势
    [self createGesture];
...
{
    if ( [self.delegate respondsToSelector:@selector(fullSreenBtnClick:button:)]) {
        [self.delegate fullSreenBtnClick:self button:sender];
    }
    UIDeviceOrientation orientation = [UIDevice currentDevice].orientation;
    UIInterfaceOrientation interfaceOrientation = (UIInterfaceOrientation)orientation;
    switch (interfaceOrientation) {
