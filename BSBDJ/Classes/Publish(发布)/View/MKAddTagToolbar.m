...
@implementation MKAddTagToolbar
...
- (void)createTagLabels:(NSArray *)tags
{
    ...
    for (int i = 0; i<tags.count; i++) {
        ...
        tagLabel.backgroundColor = MKTagBg;
        tagLabel.textAlignment = NSTextAlignmentCenter;
        tagLabel.text = tags[i];
        tagLabel.font = MKTagFont;
        // 应该要先设置文字和字体后，再进行计算
        [tagLabel sizeToFit];
        tagLabel.width += 2 * MKTagMargin;
        tagLabel.height = MKTagH;
        ...
        if (i == 0) { // 最前面的标签
            ...
            // 计算当前行左边的宽度
            CGFloat leftWidth = CGRectGetMaxX(lastTagLabel.frame) + MKTagMargin;
            // 计算当前行右边的宽度
            ...
            if (rightWidth >= tagLabel.width) { // 按钮显示在当前行
                ...
                tagLabel.x = 0;
                tagLabel.y = CGRectGetMaxY(lastTagLabel.frame) + MKTagMargin;
            }
        }
    }
    ...
    if (self.topView.width - leftWidth >= self.addButton.width) {
        ...
        self.addButton.x = 0;
        self.addButton.y = CGRectGetMaxY(lastTagLabel.frame) + MKTagMargin;
    }
    
    MKLog(@"%zd", self.tagLabels.count);
